...
$repository = 'LANCommander/LANCommander.Redistributables.OpenALSoft'
...
$staging = New-Item -ItemType Directory -Force -Path (Join-Path $temp "OpenALSoft-$version")
$package = Join-Path $temp "OpenALSoft-$version.lcx"
$payload = Join-Path $temp "OpenALSoft-$version-payload.zip"
